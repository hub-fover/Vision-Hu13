...
PREVIEW_HTML_OUTPUT = ROOT / "article" / "article_排版_石墨极简(graphite-minimal)_预览.html"
PUBLIC_BASE = "https://hub-fover.github.io/Vision-Hu13/lab-003/assets/"
...
def build_gzh(blocks, *, image_base: str | None = None, include_title: bool = False) -> str:
  ...
  <p style="font-size:18px;font-weight:700;color:#27272A;margin:0 0 8px;line-height:1.7;letter-spacing:0;">{inline(quote)}</p>
    ...
    for kind, value in blocks:
        ...
        if kind == "section":
            ...
            if value.startswith("LAB 系列承诺："):
                output.append(f'<section style="margin:38px 10px 20px;padding:20px 22px;background:#FAFAFA;border-top:2px solid #27272A;"><p style="font-size:11px;color:#A1A1AA;letter-spacing:2px;margin:0 0 9px;"><span leaf="">LAB 系列承诺</span></p><p style="margin:0;font-size:14px;line-height:1.8;color:#52525B;">{inline(value.split("：", 1)[1])}</p></section>')
            elif value.startswith("我是 Vision Hu13"):
                output.append(f'<section style="margin:0 10px;padding:26px 12px 12px;border-top:1px solid #E4E4E7;text-align:center;"><p style="font-size:15px;font-weight:800;color:#27272A;margin:0;">{inline(value)}</p></section>')
            elif value.startswith("如果这次实验对你有帮助"):
                output.append(f'<section style="padding:0 22px 24px;text-align:center;"><p style="font-size:13px;color:#64748B;margin:0;line-height:1.8;">{inline(value)}</p></section>')
            elif value == "END":
  ...
  <span style="height:1px;width:48px;background:#E4E4E7;margin-left:16px;"><span leaf=""><br></span></span>
            ...
            if image_base:
                ...
                if "assets/figures/" in source_path:
                    image_source = f"{image_base}figures/{Path(source).name}"
                elif "assets/public/" in source_path:
                    image_source = f"{image_base}public/{Path(source).name}"
                else:
                    image_source = f"{image_base}{Path(source).name}"
            else:
                image_source = source
...
def build_preview_page(blocks) -> str:
    content = build_gzh(blocks)
    return f'''<!doctype html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>LAB 003 · 公众号排版预览</title>
<style>
body{{margin:0;background:#eef0f2;font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;}}
...
.gzh-hint{{font-size:13px;color:#6b7280;line-height:1.4;}} .gzh-copy{{background:#059669;color:#fff;border:0;border-radius:9px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;}}
.gzh-stage{{max-width:700px;margin:78px auto 64px;padding:0 8px;}} .gzh-toast{{position:fixed;top:66px;left:50%;transform:translateX(-50%);background:#111827;color:#fff;padding:11px 20px;border-radius:10px;font-size:14px;opacity:0;z-index:100;}} .gzh-toast.show{{opacity:1;}}
@media(max-width:520px){{.gzh-hint{{max-width:150px;}}}}
</style></head>
<body><div class="gzh-toolbar"><span class="gzh-hint">下方是排版效果 · 点右侧复制直接粘到公众号</span><button class="gzh-copy" id="gzhCopyBtn" type="button">复制到公众号</button></div><div class="gzh-toast" id="gzhToast"></div><div class="gzh-stage"><div id="gzh-content">{content}</div></div>
<script>function gzhShowToast(message){{const toast=document.getElementById("gzhToast");toast.textContent=message;toast.classList.add("show");setTimeout(()=>toast.classList.remove("show"),2800);}}document.getElementById("gzhCopyBtn").addEventListener("click",()=>{{const range=document.createRange();range.selectNodeContents(document.getElementById("gzh-content"));const selection=window.getSelection();selection.removeAllRanges();selection.addRange(range);let copied=false;try{{copied=document.execCommand("copy");}}catch{{}}selection.removeAllRanges();gzhShowToast(copied?"已复制，可直接粘贴到公众号编辑器。":"自动复制失败，请手动全选后复制。");}});</script></body></html>\n'''


def ensure_qr() -> None:
    target = ROOT / "assets" / "public" / "lab-003-qr.png"
    target.parent.mkdir(parents=True, exist_ok=True)
    matrix = cv2.QRCodeEncoder_create().encode(LAB_URL)
    quiet = cv2.copyMakeBorder(matrix, 4, 4, 4, 4, cv2.BORDER_CONSTANT, value=255)
    size = quiet.shape[0] * 15
    image = cv2.resize(quiet, (size, size), interpolation=cv2.INTER_NEAREST)
    if not cv2.imwrite(str(target), image):
        raise RuntimeError(f"cannot write QR code: {target}")
...
def build_copy_page(blocks) -> str:
    content = build_gzh(blocks, image_base=PUBLIC_BASE, include_title=True)
...
def main() -> None:
    ...
    PREVIEW_HTML_OUTPUT.write_text(build_preview_page(blocks), encoding="utf-8")
