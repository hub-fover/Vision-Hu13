...
import { buildSampleFrames, buildSampleFlowSeries, measureMotions } from '../js/measurement.js';
...
test('deterministic sample returns speed report', () => { const result = measureMotions(buildSampleFlowSeries(120, 30), { roi: { x: 160, y: 90, width: 320, height: 180 }, scale: { p1: [80, 300], p2: [280, 300], realDistance: 1, unit: 'm' }, fps: 30 }); assert.equal(result.schemaVersion, CONTRACTS.schemaVersion); assert.ok(result.velocityMps > 0); assert.ok(result.validRatio > .9); assert.ok(Array.isArray(result.samples)); });
