...
test('sample exposes corners and actionable runtime prerequisite',async({page})=>{
  await page.goto('./'); await page.getByRole('button',{name:'用样例体验'}).click();
  await expect(page.locator('#estimateButton')).toBeEnabled(); await page.locator('#estimateButton').click();
  await expect(page.locator('#status')).toContainText(/OpenCV|构建|姿态/);
});
test('permission denial has recovery text',async({page})=>{
  await page.goto('./'); await page.getByRole('button',{name:'实时跟踪'}).click();
  await page.getByRole('button',{name:'开始相机'}).click(); await expect(page.locator('#liveStatus')).toContainText(/权限|不支持/);
});
test('calibration import rejects malformed JSON',async({page})=>{
  await page.goto('./'); await page.getByRole('button',{name:'相机标定'}).click();
