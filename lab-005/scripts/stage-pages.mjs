...
export async function validatePagesStage(destination = defaultDestination) {
  ...
  if (!Array.isArray(runtimeManifest.requiredModules) || !runtimeManifest.requiredModules.includes("core") || !runtimeManifest.requiredModules.includes("imgproc")) throw new Error("runtime manifest must include core and imgproc");
  if (runtimeManifest.capabilities?.checkerboardFallback !== true || runtimeManifest.capabilities?.calibrateCameraExtended !== true) throw new Error("runtime manifest must declare the browser checkerboard fallback capabilities");
  if (!runtimeManifest.calibrationFallback) throw new Error("runtime manifest must document calibration fallback");
